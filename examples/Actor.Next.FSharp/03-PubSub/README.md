...
```powershell
cd examples\Actor.Next.FSharp\03-PubSub
dapr run --app-id actors-example-03-fsharp --resources-path .\components --dapr-grpc-port 50001 --dapr-http-port 3500 --app-protocol grpc --app-port 5056
```
...
```powershell
cd examples\Actor.Next.FSharp\03-PubSub\Cart.Next.FSharp.Example03.Host
dotnet run
```
...
```powershell
dotnet test Cart.Next.FSharp.Example03.Tests\Cart.Next.FSharp.Example03.Tests.fsproj --no-restore
```
