...
{
    private DateVersionStrategyOptions options = new();
    ...
    public bool TryParse(string typeName, out string canonicalName, out string version)
    {
        ...
        var format = string.IsNullOrWhiteSpace(options.DateFormat) ? "yyyyMMdd" : options.DateFormat;
        ...
        var candidate = typeName[suffixStart..];
        if (!TryParseDate(candidate, format, options.IgnorePrefixCase, out _))
        {
            return ApplyNoSuffix(typeName, prefix, comparison, out canonicalName, out version);
        }
        ...
        {
            canonicalName = typeName[..suffixStart];
        }
    ...
    public int Compare(string? v1, string? v2)
    {
        ...
        var format = string.IsNullOrWhiteSpace(options.DateFormat) ? "yyyyMMdd" : options.DateFormat;
        var ok1 = TryParseDate(v1.Trim(), format, options.IgnorePrefixCase, out var d1);
        var ok2 = TryParseDate(v2.Trim(), format, options.IgnorePrefixCase, out var d2);
    ...
    private static bool TryParseDate(string value, string format, bool ignorePrefixCase, out DateTime date)
    {
        ...
        if (!string.Equals(upper, value, StringComparison.Ordinal) &&
            DateTime.TryParseExact(upper, format, CultureInfo.InvariantCulture, DateTimeStyles.None, out date))
        {
            return true;
        }
        ...
        {
            return DateTime.TryParseExact(lower, format, CultureInfo.InvariantCulture, DateTimeStyles.None, out date);
        }
