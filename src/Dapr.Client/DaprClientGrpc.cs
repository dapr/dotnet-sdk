...
namespace Dapr.Client
{
    ...
    internal class DaprClientGrpc : DaprClient
    {
            ...
            string keyName, EncryptionOptions encryptionOptions, CancellationToken cancellationToken = default)
        {
            ...
            await using (var bufferedStream = new BufferedStream(plaintextStream, (int)encryptionOptions.StreamingBlockSizeInBytes))
            {
                var buffer = new byte[encryptionOptions.StreamingBlockSizeInBytes];
                int bytesRead;
                ulong sequenceNumber = 0;

                while ((bytesRead = await bufferedStream.ReadAsync(buffer, 0, (int)encryptionOptions.StreamingBlockSizeInBytes, cancellationToken)) != 0)
                {
        ...
        public override async Task<ReadOnlyMemory<byte>> DecryptAsync(string vaultResourceName, Stream ciphertextStream, string keyName, DecryptionOptions decryptionOptions, CancellationToken cancellationToken = default)
        {
            ...
            //Send the cipher text bytes in configurably-sized blocks in subsequent messages
            await using (var bufferedStream = new BufferedStream(ciphertextStream, (int)decryptionOptions.StreamingBlockSizeInBytes))
            {
                var buffer = new byte[(int)decryptionOptions.StreamingBlockSizeInBytes];
                int bytesRead;
                ulong sequenceNumber = 0;

                while ((bytesRead = await bufferedStream.ReadAsync(buffer, 0, (int)decryptionOptions.StreamingBlockSizeInBytes, cancellationToken)) != 0)
                {
                    await duplexStream.RequestStream.WriteAsync(
