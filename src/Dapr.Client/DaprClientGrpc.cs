...
namespace Dapr.Client
{
    ...
    internal class DaprClientGrpc : DaprClient
    {
            ...
            string decryptionKeyName, bool omitDecryptionKey, CancellationToken cancellationToken = default)
        {
            ...
            ArgumentVerifier.ThrowIfNull(stream, nameof(stream));

            var encryptRequestOptions = new Autogenerated.EncryptRequestOptions
            {
                ComponentName = vaultResourceName,
                DataEncryptionCipher = dataEncryptionCipher.GetValueFromEnumMember(),
                KeyName = keyName,
                KeyWrapAlgorithm = algorithm.GetValueFromEnumMember(),
                OmitDecryptionKeyName = omitDecryptionKey
            };

            if (!omitDecryptionKey)
            {
                ArgumentVerifier.ThrowIfNullOrEmpty(decryptionKeyName, nameof(decryptionKeyName));
                encryptRequestOptions.DecryptionKeyName = decryptionKeyName;
            }
            ...
            //Send the plaintext bytes in 4 kb blocks in subsequent messages
            const int blockSize = 4 * 1024; // 4 kb
            await using (var bufferedStream = new BufferedStream(stream, blockSize))
            {
                ...
                while ((bytesRead = await bufferedStream.ReadAsync(buffer, 0, blockSize, cancellationToken)) != 0)
                {
        ...
        private async Task<byte[]> EncryptByteArrayAsync(string vaultResourceName, byte[] plainTextBytes,
            KeyWrapAlgorithm algorithm, string keyName, DataEncryptionCipher dataEncryptionCipher,
            string decryptionKeyName, bool omitDecryptionKey, CancellationToken cancellationToken = default)
        {
            ArgumentVerifier.ThrowIfNullOrEmpty(vaultResourceName, nameof(vaultResourceName));
            ...
            //Send the plaintext bytes in 4 kb blocks in subsequent messages
            const int blockSize = 4 * 1024; // 4 kb
            var blockCount = (int)Math.Ceiling((double)plainTextBytes.Length / blockSize);

            for (var a = 0; a < blockCount; a++)
            {
                var offset = a * blockSize;
                var count = Math.Min(blockSize, plainTextBytes.Length - offset);
                
                await duplexStream.RequestStream.WriteAsync(new Autogenerated.EncryptRequest
                {
                    Payload = new Autogenerated.StreamPayload
                    {
                        Data = ByteString.CopyFrom(plainTextBytes, offset, count),
                        Seq = (ulong)a
                    }
                }, cancellationToken);
            }
        ...
        public override async Task<byte[]> DecryptAsync(string vaultResourceName, byte[] cipherTextBytes, string keyName, CancellationToken cancellationToken = default)
        {
            ...
            //Send the cipher text bytes in 4 kb blocks in subsequent messages
            const int blockSize = 4 * 1024; // 4 kb
            var blockCount = (int)Math.Ceiling((double)cipherTextBytes.Length / blockSize);

            for (var a = 0; a < blockCount; a++)
            {
                var offset = a * blockSize;
                var count = Math.Min(blockSize, cipherTextBytes.Length - offset);
            ...
            //Set up how the response is handled since we know it's of finite length
            var returnedBytes = new List<byte>();
            await foreach (var decryptedResponse in duplexStream.ResponseStream.ReadAllAsync(cancellationToken)
                               .ConfigureAwait(false))
            {
                returnedBytes.AddRange(decryptedResponse.Payload.Data.ToByteArray());
            }
        ...
        public override async Task<byte[]> DecryptAsync(string vaultResourceName, Stream cipherTextStream, string keyName, CancellationToken cancellationToken = default)
        {
            ...
            await using (var bufferedStream = new BufferedStream(cipherTextStream, blockSize))
            {
                var buffer = new byte[blockSize];
                int bytesRead;
                ulong sequenceNumber = 0;
