...
namespace Dapr.Client
{
    ...
    internal class DaprClientGrpc : DaprClient
    {
        ...
        /// <inheritdoc/>
        public override async IAsyncEnumerable<byte[]> EncryptAsync(byte[] plainTextBytes, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            var options = CreateCallOptions(headers: null, cancellationToken);
            var resp = client.EncryptAlpha1(options);

            await foreach (var result in resp.ResponseStream.ReadAllAsync(cancellationToken))
            {
                yield return result.ToByteArray();
            }
        }

        /// <inheritdoc/>
        public override async IAsyncEnumerable<byte[]> DecryptAsync(byte[] cipherTextBytes, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            Autogenerated.DecryptRequest request = new Autogenerated.DecryptRequest() { };
            var options = CreateCallOptions(headers: null, cancellationToken);

            var resp = client.DecryptAlpha1(options);
