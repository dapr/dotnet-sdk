...
namespace Dapr
{
    ...
    public abstract class StateClient
    {
        ...
        public abstract ValueTask<TValue> GetStateAsync<TValue>(string stateStoreName, string key, CancellationToken cancellationToken = default);
        ...
        public async ValueTask<StateEntry<TValue>> GetStateEntryAsync<TValue>(string stateStoreName, string key, CancellationToken cancellationToken = default)
        {
            ...
            var value = await this.GetStateAsync<TValue>(stateStoreName, key, cancellationToken);
            return new StateEntry<TValue>(this, stateStoreName, key, value);
        }
        ...
        public abstract ValueTask SaveStateAsync<TValue>(string stateStoreName, string key, TValue value, CancellationToken cancellationToken = default);
        ...
        /// <param name="cancellationToken">A <see cref="CancellationToken" /> that can be used to cancel the operation.</param>
        /// <returns>A <see cref="ValueTask" /> that will complete when the operation has completed.</returns>
        public abstract ValueTask DeleteStateAsync(string stateStoreName, string key, CancellationToken cancellationToken = default);
    }
}
