...
internal sealed class DaprDistributedLockGrpcClient(Autogenerated.Dapr.DaprClient client, string daprApiToken) : DaprDistributedLockClient
{
    /// <inheritdoc />
    public override async Task<LockResponse> TryLockAsync(string storeName, string resourceId, string lockOwner, int expiryInSeconds,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var grpcCallOptions =
                DaprClientUtilities.ConfigureGrpcCallOptions(typeof(DaprDistributedLockClient).Assembly, daprApiToken,
                    cancellationToken);
            var response = await client.TryLockAlpha1Async(request, grpcCallOptions);
            if (!response.Success)
            {
                throw new DaprException(
                    "Lock operation failed: the Dapr endpoint did not indicate a lock was applied successfully.");
            }
            
            return new LockResponse(this) { StoreName = storeName, ResourceId = resourceId, LockOwner = lockOwner };
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var grpcCallOptions =
                DaprClientUtilities.ConfigureGrpcCallOptions(typeof(DaprDistributedLockClient).Assembly, daprApiToken,
                    cancellationToken);
            response = await client.UnlockAlpha1Async(request, grpcCallOptions);
        }
        catch (RpcException ex)
        {
            throw new DaprException(
