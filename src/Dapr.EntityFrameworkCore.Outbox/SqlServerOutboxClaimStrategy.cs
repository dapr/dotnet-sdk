...
/// <summary>
/// SQL Server–optimized claim strategy. Uses <c>WITH (ROWLOCK, UPDLOCK, READPAST)</c> plus
/// <c>OUTPUT INSERTED.*</c> so multiple dispatcher replicas can drain the outbox concurrently
/// without deadlocks or double-delivery.
/// </summary>
public sealed class SqlServerOutboxClaimStrategy : IOutboxClaimStrategy
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var sql =
            ...
            $"    SET t.[LockOwner] = @owner,\n" +
            $"        t.[LockedUntil] = @lockedUntil,\n" +
            $"        t.[AttemptCount] = t.[AttemptCount] + 1\n" +
            $"    OUTPUT INSERTED.*\n" +
            $"FROM {quotedTable} AS t WITH (ROWLOCK, UPDLOCK, READPAST)\n" +
    ...
    {
        // Prefer whatever the model advertises so overrides via ModelBuilder still apply;
        // fall back to the options if the entity is not registered yet.
    ...
    {
        // Read by name for resiliency to column ordering changes.
