...
public sealed class WorkflowSourceGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(allDiscovered, (spc, items) =>
        {
            ...
            var concreteWorkflows = workflows.Where(w => w is not null && !w!.IsAbstractOrGeneric).ToList();
            ...
            spc.ReportDiagnostic(Diagnostic.Create(
                new DiagnosticDescriptor(
                    "DAPRWFVER002",
                    "Final type count",
                    $"Source generator will generate registry for {concreteWorkflows.Count} workflow(s) and {activities.Count} activity/activities. To view generated code, add <EmitCompilerGeneratedFiles>true</EmitCompilerGeneratedFiles> to your project file.",
                    "Dapr.Workflow.Versioning",
                    DiagnosticSeverity.Info,
                    isEnabledByDefault: true),
                Location.None));
