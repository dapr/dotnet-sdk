...
public class BulkPublishEventApiTest
{
    ...
    public async Task BulkPublishEventAsync_WrapsRpcException()
    {
        ...
        {
            await client.DaprClient.BulkPublishEventAsync(TestPubsubName, TestTopicName, bulkPublishData, cancellationToken: TestContext.Current.CancellationToken);
        });
    ...
    [Fact]
    public async Task BulkPublishEventAsync_CanPublishTopicWithByteArrayEvents()
    {
        await using var client = TestClient.CreateForDaprClient();

        var event1 = System.Text.Encoding.UTF8.GetBytes("hello");
        var event2 = System.Text.Encoding.UTF8.GetBytes("world");
        var events = new List<byte[]>() { event1, event2 };

        var request = await client.CaptureGrpcRequestAsync(async daprClient =>
            await daprClient.BulkPublishEventAsync(TestPubsubName, TestTopicName, events));
        ...
        var firstEntry = envelope.Entries[0];
        firstEntry.EntryId.ShouldBe("0");
        firstEntry.ContentType.ShouldBe(Constants.ContentTypeApplicationOctetStream);
        firstEntry.Event.ToByteArray().ShouldBe(event1);
        firstEntry.Metadata.ShouldBeEmpty();

        var secondEntry = envelope.Entries[1];
        secondEntry.EntryId.ShouldBe("1");
        secondEntry.ContentType.ShouldBe(Constants.ContentTypeApplicationOctetStream);
        secondEntry.Event.ToByteArray().ShouldBe(event2);
        secondEntry.Metadata.ShouldBeEmpty();
