...
public class DaprApiTokenTest
{
    ...
    [Fact]
    public async Task DeleteBulkStateAsync_WithApiTokenSet_IncludesTokenHeader()
    {
        await using var client = TestClient.CreateForDaprClient(c => c.UseDaprApiToken("test_token"));

        var items = new List<BulkDeleteStateItem> { new BulkDeleteStateItem("key1", "etag1") };
        var request = await client.CaptureGrpcRequestAsync(async daprClient =>
        {
            await daprClient.DeleteBulkStateAsync("testStore", items);
        });

        request.Dismiss();

        await request.GetRequestEnvelopeAsync<Autogenerated.DeleteBulkStateRequest>();
    ...
    [Fact]
    public async Task DeleteBulkStateAsync_WithoutApiToken_OmitsTokenHeader()
    {
        await using var client = TestClient.CreateForDaprClient();
