...
public sealed class DaprClientBuilderExtensionsTests
{
    ...
    public void AddDaprClient_CallsConfigureAction()
    {
        ...
        services.AddDaprClient<DaprTestClient, DaprTestGrpcClient, DaprTestBuilder, DaprTestClientBuilder>(
            config => new DaprTestClientBuilder(config),
            svc => new DaprTestBuilder(svc),
            (_, _) =>
        {
...
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    private sealed class DaprTestClientBuilder(IConfiguration? configuration = null) : DaprGenericClientBuilder<DaprTestClient>(configuration)
    {
        ...
        {
            var deps = this.BuildDaprClientDependencies(typeof(DaprTestClient).Assembly);
            var daprClient = new Autogenerated.Dapr.DaprClient(deps.channel);
            IDaprRuntimeCapabilities runtimeCapabilities = new DaprRuntimeCapabilities(deps.channel);
            return new DaprTestGrpcClient(daprClient, deps.httpClient, runtimeCapabilities, deps.daprApiToken);
        }
    }
