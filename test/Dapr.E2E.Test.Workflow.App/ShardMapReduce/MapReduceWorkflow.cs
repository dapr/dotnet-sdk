...
public sealed class MapReduceWorkflow : Workflow<MapReduceInput, MapReduceResult> 
{
    public override async Task<MapReduceResult> RunAsync(WorkflowContext context, MapReduceInput input)
    {
        ...
        context.SetCustomStatus(new
        {
            Phase = "RunningShards",
            input.ShardCount,
            MaxParallelShards = input.ShardBatchSize
        });
