...
public class DaprConfigurationStoreProviderTest
{
    ...
    {
        var streamBlocked = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        daprClient
            .Setup(c => c.SubscribeConfiguration(
                "store",
                It.IsAny<IReadOnlyList<string>>(),
                It.IsAny<IReadOnlyDictionary<string, string>>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(new SubscribeConfigurationResponse(new BlockingConfigurationSource(streamBlocked)));

        var config = new ConfigurationBuilder()
            .AddStreamingDaprConfigurationStore("store", new List<string>(), daprClient.Object, TimeSpan.FromMilliseconds(100))
            .Build();

        await streamBlocked.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        config["testKey"].ShouldBe("testValue");

        (config as IDisposable)?.Dispose();
        await Task.Delay(TimeSpan.FromMilliseconds(100), TestContext.Current.CancellationToken);

        daprClient.Verify(c => c.UnsubscribeConfiguration(
            "store",
            "testId",
            It.IsAny<CancellationToken>()), Times.Never());
        daprClient.Verify(c => c.SubscribeConfiguration(
            "store",
            It.IsAny<IReadOnlyList<string>>(),
            It.IsAny<IReadOnlyDictionary<string, string>>(),
    ...
    private sealed class BlockingConfigurationSource(TaskCompletionSource<bool> streamBlocked) : ConfigurationSource
    {
        ...
        {
            return new BlockingConfigurationEnumerator(streamBlocked, cancellationToken);
        }
    }

    private sealed class BlockingConfigurationEnumerator(TaskCompletionSource<bool> streamBlocked, CancellationToken cancellationToken) : IAsyncEnumerator<IDictionary<string, ConfigurationItem>>
    {
        private bool hasReturnedInitialValue;

        public IDictionary<string, ConfigurationItem> Current { get; } = new Dictionary<string, ConfigurationItem>
        {
            ["testKey"] = new("testValue", "v1", null)
        };
