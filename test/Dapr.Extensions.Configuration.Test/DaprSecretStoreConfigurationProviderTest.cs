...
namespace Dapr.Extensions.Configuration.Test
{
    ...
    public class DaprSecretStoreConfigurationProviderTest
    {
        ...
        [Fact]
        public void DaprClient_InjectDuringRegistration()
        {
            //Build out a DI registered DaprClient instance
            var httpClient = new TestHttpClient()
            {
                Handler = async (entry) =>
                {
            ...
            services.AddSingleton(_ => daprClient);
            var serviceProvider = services.BuildServiceProvider();

            //Build out the configuration
            var config = CreateBuilder()
                .AddDaprSecretStore(serviceProvider, (client, conf) =>
                {
                    Assert.Equal(17, client.JsonSerializerOptions.MaxDepth);
