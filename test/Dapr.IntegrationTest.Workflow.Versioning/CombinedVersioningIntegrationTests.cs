...
public sealed class CombinedVersioningIntegrationTests
{
    ...
    public async Task ShouldCombinePatchAndNameBasedVersioning()
    {
        ...
        var appId = $"workflow-versioning-combined-{Guid.NewGuid():N}";
                     ...
                     await StartVersionedAppAsync(componentsDirV1, _ => new MinVersionSelector(), environment, options))
        {
            ...
            Assert.Equal(nameof(CombinedWorkflowV1), latestNameV1);

            await client1.ScheduleNewWorkflowAsync(latestNameV1, instanceIdV1, 5);
        ...
        var app = await DaprHarnessBuilder.ForHarness(harness)
            .ConfigureServices(builder =>
            {
                ...
                builder.Services.AddDaprWorkflowBuilder(
                    ...
                    {
                        opt.RegisterWorkflow<CombinedWorkflowV1>();
                        opt.RegisterWorkflow<CombinedWorkflowV2>();
                    },
    ...
    internal sealed class CombinedWorkflowV1 : Workflow<int, string>
    {
        public override async Task<string> RunAsync(WorkflowContext context, int input)
        {
            ...
            return $"v1:{value}:{resume}";
        }
    }

    [WorkflowVersion(CanonicalName = CanonicalWorkflowName, Version = "2")]
    internal sealed class CombinedWorkflowV2 : Workflow<int, string>
    {
        public override async Task<string> RunAsync(WorkflowContext context, int input)
        {
            var value = input;
            ...
            if (context.IsPatched("p2"))
            {
                value += 10;
            }

            var resume = await context.WaitForExternalEventAsync<string>(ResumeEventName);
            return $"v2:{value}:{resume}";
        }
    }
