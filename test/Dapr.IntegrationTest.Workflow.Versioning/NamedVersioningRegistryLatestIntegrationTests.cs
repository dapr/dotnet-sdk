...
public sealed class NamedVersioningRegistryLatestIntegrationTests
{
    private const string CanonicalWorkflowName = "RegistryLatestWorkflow";
    private const string ResumeEventName = "resume";

    [MinimumDaprRuntimeFact("1.17")]
    public async Task ShouldUseLatestRegistryNameForNewInstanceAndPersistAcrossRestart()
    {
        var instanceId = Guid.NewGuid().ToString("N");
        var appId = $"workflow-versioning-latest-{Guid.NewGuid():N}";
        var options = new DaprRuntimeOptions().WithAppId(appId);
        var componentsDirV1 = TestDirectoryManager.CreateTestDirectory("workflow-versioning-latest-v1");
        var componentsDirV2 = TestDirectoryManager.CreateTestDirectory("workflow-versioning-latest-v2");
                     ...
                     await StartVersionedAppAsync(componentsDirV2, _ => new MaxVersionSelector(), environment, options))
        {
            ...
            Assert.Equal(nameof(RegistryLatestWorkflowV2), latestNameV2);

            await client2.ScheduleNewWorkflowAsync(latestNameV2, instanceId, "latest");
            using (var startCts = new CancellationTokenSource(TimeSpan.FromMinutes(1)))
            {
                try
                {
                    await client2.WaitForWorkflowStartAsync(instanceId, cancellation: startCts.Token);
                }
                catch (OperationCanceledException)
                {
                    var state = await client2.GetWorkflowStateAsync(instanceId, getInputsAndOutputs: false);
                     ...
                     await StartVersionedAppAsync(componentsDirV1, _ => new MinVersionSelector(), environment, options))
        {
            ...
            Assert.Equal(nameof(RegistryLatestWorkflowV1), latestNameV1);

            await client1.RaiseEventAsync(instanceId, ResumeEventName, "resume");
            using var resumeCts = new CancellationTokenSource(TimeSpan.FromMinutes(2));
            var resumed = await client1.WaitForWorkflowCompletionAsync(instanceId, cancellation: resumeCts.Token);
            Assert.Equal(WorkflowRuntimeStatus.Completed, resumed.RuntimeStatus);
            Assert.Equal("v2:latest:resume", resumed.ReadOutputAs<string>());
        }
    }
    ...
    internal sealed class RegistryLatestWorkflowV1 : Workflow<string, string>
    {
    ...
    internal sealed class RegistryLatestWorkflowV2 : Workflow<string, string>
    {
        public override async Task<string> RunAsync(WorkflowContext context, string input)
        {
            var resume = await context.WaitForExternalEventAsync<string>(ResumeEventName);
