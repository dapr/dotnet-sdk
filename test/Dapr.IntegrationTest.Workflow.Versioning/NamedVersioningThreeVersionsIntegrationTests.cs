...
public sealed class NamedVersioningThreeVersionsIntegrationTests
{
    private const string CanonicalWorkflowName = "AttributeOverrideWorkflowThree";
    private const string ResumeEventName = "resume";
    
    [MinimumDaprRuntimeFact("1.17")]
    public async Task ShouldRouteAcrossThreeVersionsWithAttributeOverride()
    {
        var instanceIdV1 = Guid.NewGuid().ToString("N");
        var instanceIdV2 = Guid.NewGuid().ToString("N");
        var instanceIdV3 = Guid.NewGuid().ToString("N");
        var appId = $"workflow-versioning-three-{Guid.NewGuid():N}";
        var options = new DaprRuntimeOptions().WithAppId(appId);
        var componentsDirV1 = TestDirectoryManager.CreateTestDirectory("workflow-versioning-three-v1");
        var componentsDirV2 = TestDirectoryManager.CreateTestDirectory("workflow-versioning-three-v2");
        var componentsDirV3 = TestDirectoryManager.CreateTestDirectory("workflow-versioning-three-v3");
                     ...
                     await StartVersionedAppAsync(componentsDirV1, _ => new MinVersionSelector(), environment, options))
        {
            ...
            Assert.Equal(nameof(OverrideWorkflowOne), latestNameV1);
        ...
        await using (var appV2 =
                     await StartVersionedAppAsync(componentsDirV2, _ => new MidVersionSelector(), environment, options))
        {
            ...
            Assert.Equal(nameof(OverrideWorkflowTwo), latestNameV2);
            ...
            using var midCts = new CancellationTokenSource(TimeSpan.FromMinutes(2));
            var middle = await client2.WaitForWorkflowCompletionAsync(instanceIdV2, cancellation: midCts.Token);
            Assert.Equal(WorkflowRuntimeStatus.Completed, middle.RuntimeStatus);
            Assert.Equal("v2:second:resume", middle.ReadOutputAs<string>());
        }

        await using (var appV3 =
                     await StartVersionedAppAsync(componentsDirV3, _ => new MaxVersionSelector(), environment, options))
        {
            using var scope3 = appV3.CreateScope();
            var client3 = scope3.ServiceProvider.GetRequiredService<DaprWorkflowClient>();

            var latestNameV3 = GetLatestWorkflowName(scope3.ServiceProvider);
            Assert.Equal(nameof(OverrideWorkflowThree), latestNameV3);

            await client3.ScheduleNewWorkflowAsync(latestNameV3, instanceIdV3, new Payload("third"));
            await client3.RaiseEventAsync(instanceIdV3, ResumeEventName, "resume");
            using var latestCts = new CancellationTokenSource(TimeSpan.FromMinutes(2));
            var latest = await client3.WaitForWorkflowCompletionAsync(instanceIdV3, cancellation: latestCts.Token);
            Assert.Equal(WorkflowRuntimeStatus.Completed, latest.RuntimeStatus);
            Assert.Equal("v3:third:resume", latest.ReadOutputAs<string>());
        }
    }
        ...
        var app = await DaprHarnessBuilder.ForHarness(harness)
            .ConfigureServices(builder =>
            {
                ...
                builder.Services.AddDaprWorkflowBuilder(
                    ...
                    {
                        opt.RegisterWorkflow<OverrideWorkflowOne>();
                        opt.RegisterWorkflow<OverrideWorkflowTwo>();
                        opt.RegisterWorkflow<OverrideWorkflowThree>();
                    },
    ...
    internal sealed class OverrideWorkflowOne : Workflow<Payload, string>
    {
    ...
    internal sealed class OverrideWorkflowTwo : Workflow<Payload, string>
    {
        public override async Task<string> RunAsync(WorkflowContext context, Payload input)
        {
            var resume = await context.WaitForExternalEventAsync<string>(ResumeEventName);
    ...
    [WorkflowVersion(CanonicalName = CanonicalWorkflowName, Version = "17")]
    internal sealed class OverrideWorkflowThree : Workflow<Payload, string>
    {
        public override async Task<string> RunAsync(WorkflowContext context, Payload input)
        {
            var resume = await context.WaitForExternalEventAsync<string>(ResumeEventName);
            return $"v3:{input.Name}:{resume}";
        }
    }
    ...
    private sealed class MidVersionSelector : IWorkflowVersionSelector
    {
        public WorkflowVersionIdentity SelectLatest(
            string canonicalName,
            IReadOnlyCollection<WorkflowVersionIdentity> candidates,
            IWorkflowVersionStrategy strategy)
        {
            ...
            var ordered = candidates.OrderBy(v => v.Version, strategy).ToList();
