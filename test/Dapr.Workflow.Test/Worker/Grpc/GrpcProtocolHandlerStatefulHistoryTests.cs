...
public sealed class GrpcProtocolHandlerStatefulHistoryTests
{
    ...
    public async Task CompletedWorkflow_EvictsCacheEntry_SoTheNextDeltaMisses()
    {
        ...
        // Turn 1 warms the cache but ends the instance, so its entry must be dropped. Turn 2 arrives
        // as a delta the cache would otherwise have satisfied; it has to fall back instead.
        var turn1 = new WorkItem { WorkflowRequest = new WorkflowRequest { InstanceId = "i-1", PastEvents = { Events(2) } } };
        var delta = new WorkflowRequest { InstanceId = "i-1", PastEvents = { Events(1) } };
        delta.CachedHistory = new CachedHistory { EventCount = 2 };
        var turn2 = new WorkItem { WorkflowRequest = delta };
        ...
        grpcClientMock
            .Setup(x => x.GetInstanceHistoryAsync(It.IsAny<GetInstanceHistoryRequest>(), It.IsAny<CallOptions>()))
            .Callback(() => Interlocked.Increment(ref fetchCount))
            .Returns(CreateAsyncUnaryCall(new GetInstanceHistoryResponse { Events = { Events(9) } }));
        ...
        await RunHandlerUntilAsync(handler,
            ...
            {
                seenPastEvents.Enqueue(req.PastEvents.Count);
                var response = new WorkflowResponse { InstanceId = req.InstanceId };

                // Only the first turn ends the instance; the second must not re-add a CompleteWorkflow
    ...
    [Fact]
    public async Task Reconnect_ResetsCache_SoTheNextDeltaMisses()
    {
        var grpcClientMock = CreateGrpcClientMock();

        // The sidecar drops a stream's warm set when it disconnects, so the worker must start the next
        // stream cold. Turn 1 lands on the first stream; the stream then ends and turn 2 arrives on a
        // fresh one as a delta whose prefix the (now reset) cache must no longer claim to hold.
        var turn1 = new WorkItem { WorkflowRequest = new WorkflowRequest { InstanceId = "i-1", PastEvents = { Events(2) } } };
        var delta = new WorkflowRequest { InstanceId = "i-1", PastEvents = { Events(1) } };
        delta.CachedHistory = new CachedHistory { EventCount = 2 };
        var turn2 = new WorkItem { WorkflowRequest = delta };
        ...
        grpcClientMock
            ...
            .Returns(CreateServerStreamingCall([turn1]))
            .Returns(CreateServerStreamingCall([turn2]))
            .Returns(CreateServerStreamingCall([]));
        ...
        // The handler waits ReconnectDelay (5s) between streams, so allow well past that.
        await RunHandlerUntilAsync(handler,
            workflowHandler: (req, _) =>
            {
                seenPastEvents.Enqueue(req.PastEvents.Count);
            ...
            NoActivityHandler,
            untilCondition: () => Volatile.Read(ref completed) >= 2, timeout: TimeSpan.FromSeconds(20));

        Assert.Equal([2, 9], seenPastEvents);
        Assert.Equal(1, Volatile.Read(ref fetchCount));
    }

    [Fact]
    public async Task FailedHistoryFetch_AbandonsTheTurn_WithoutCompletingIt()
    {
        var grpcClientMock = CreateGrpcClientMock();

        // A delta the cold cache cannot satisfy, whose recovery fetch then fails. There is no per-item
        // NACK, so the turn must be abandoned for backend redelivery rather than completed with a
        // partial history.
        var delta = new WorkflowRequest { InstanceId = "i-1", PastEvents = { Events(1) } };
        delta.CachedHistory = new CachedHistory { EventCount = 5 };
        grpcClientMock
            .Setup(x => x.GetWorkItems(It.IsAny<GetWorkItemsRequest>(), It.IsAny<CallOptions>()))
            .Returns(CreateServerStreamingCall([new WorkItem { WorkflowRequest = delta }]));
        ...
        var workflowHandlerInvoked = 0;
        var handler = new GrpcProtocolHandler(grpcClientMock.Object, NullLoggerFactory.Instance);

        await RunHandlerUntilAsync(handler,
            workflowHandler: (req, _) =>
            {
                Interlocked.Increment(ref workflowHandlerInvoked);
                return Task.FromResult(new WorkflowResponse { InstanceId = req.InstanceId });
            },
            NoActivityHandler,
            untilCondition: () => Volatile.Read(ref fetchCount) >= 1, timeout: TimeSpan.FromSeconds(5));

        Assert.Equal(0, Volatile.Read(ref workflowHandlerInvoked)); // never replayed
